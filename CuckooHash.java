...
public class CuckooHash<K, V> {
	...
	public void put(K key, V value) {
		...
		K currKey = key;
		V currValue = value;
		Bucket<K, V> currBucket = newBucket;

		while (i < CAPACITY) {
			int pos = useFirstHash ? hash1(currKey) : hash2(currKey);

			//bucket empty
			if (table[pos] == null) {
				table[pos] = currBucket;
				return;
			}

			Bucket<K, V> existing = table[pos];
			//return early if already exists
			if (existing.getBucKey().equals(currKey) && existing.getValue().equals(currValue)) {
				return;
			}

			table[pos] = currBucket;
			currBucket = existing;
			currKey = existing.getBucKey();
			currValue = existing.getValue();
